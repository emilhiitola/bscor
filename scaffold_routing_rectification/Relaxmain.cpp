 ...
 */
template<typename StoreBestFunctorT, typename RunningFunctorT>
void relaxation::gradient_descent(scene & mesh, physics & phys, int minbasecount, StoreBestFunctorT store_best_functor, RunningFunctorT running_functor, const int &iterations) {
	scene::HelixContainer & helices(mesh.getHelices());
...
int relaxation::relax_main(std::string input_file, std::string output_file, const double *dblArgs,
                           const int &iterations, const bool *boolArgs) {
	seed();
    ...
    str = info.str(); std::cout << str.c_str();; info.str(std::string());
